                    ...
                    @foreach($users as $key => $user)
                        @if ($user != null)
                            <tr>
                                ...
                                <td class="text-center">{{$user->wholesale  && $user->wholesale == '1' ? translate('wholesale') : translate('retail')}}</td>
                                <td class="text-center">{{$user->active  && $user->active == '1' ? translate('active') : translate('Not active')}}</td>

                                <td class="text-right">
                                    @if($user->wholesale && $user->wholesale==1)
                                        ...
                                        <div class="modal-dialog modal-lg" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">{{translate('document')}}</h5>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <div class="modal-body">
                                                    <img src="{{ static_asset($user->document) }}" class="img-fluid" alt="Image">
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                    <a href="{{route('customers.approve', encrypt($user->id))}}" class="btn btn-primary">
                                                        {{ $user->active == '0' ? translate('Approve') : translate('Not Approve') }}
                                                    </a>                                                </div>
                                            </div>
                                        </div>
                                    </div>
